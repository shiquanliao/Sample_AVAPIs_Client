...
static const char *vertexShader = GET_STR(
        attribute
        vec4 aPosition; // 顶点坐标
        attribute // 用于经常更改的变量,只能用在顶点着色器
        vec2 aTexCoord; // 材质顶点坐标
        varying // 用于修饰从顶点着色器向片元着色器传递的变量,数据传递
...
//    };
    ...
    static float vers[] = {
            width, height - 2 * offset, 0.0f,
            -1.0f, height - 2 * offset, 0.0f,
            width, 1.0f - 2 * offset, 0.0f,
            -1.0f, 1.0f - 2 * offset, 0.0f,
    };
...
//    glPixelStorei(GL_UNPACK_ROW_LENGTH, 738);
//    glPixelStorei(GL_UNPACK_SKIP_ROWS, 100);
...
void XShader::SetShaderVertex(float widthRatioForScreen, float heightRationForScreen, float offset) {
    mutex.lock();
